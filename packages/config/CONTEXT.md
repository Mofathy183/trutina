...
adapters (`trutina-storage-mongo`, and — per `PostgresSettings`' own
docstring — `trutina-storage-postgres`) that all need the same categories of
configuration — a Mongo connection, a PostgreSQL connection, API bind
settings — sourced the same way. The alternative (`os.environ` reads
...
- **One flat `Settings` model with no nesting** (`mongo_uri`, `mongo_db`,
  `postgres_uri`, `api_host`, `api_port` as top-level fields). Rejected: as
  the number of settings groups grows — `PostgresSettings` joining
  `MongoSettings`/`ApiSettings` is the confirmed example of this happening —
  a flat model becomes an undifferentiated field list with no way to tell
  which fields belong to which subsystem at a glance. Nesting
  (`settings.mongo.uri`, `settings.postgres.uri`, `settings.api.port`) keeps
  each group's fields visibly grouped and independently reusable — a storage
  adapter only ever needs its own nested settings object, not the whole
  `Settings` object.
...
- **Nested settings models (`MongoSettings`, `ApiSettings`,
  `PostgresSettings`) are plain `BaseModel`, not `BaseSettings`.** Confirmed
  in source: none of the three defines a `model_config`/
  `SettingsConfigDict`. Only the root `Settings`/`TestSettings` classes own
  env-prefix and dotenv-file configuration. This means a nested settings
  object can never be constructed standalone from the environment — it must
  always be built as part of the root model's `default_factory`. That's an
  accepted constraint, not an oversight: giving every nested model its own
...
- Every settings group (`MongoSettings`, `ApiSettings`, `PostgresSettings`,
  and any future addition) stays a plain `BaseModel` nested inside
  `Settings` — never its own independently-loaded `BaseSettings`.
...
- **No I/O beyond dotenv-file reads performed by `pydantic-settings`
  ...
  confirmed, none of `base.py`/`mongo.py`/`api.py`/`postgres.py` perform any
  I/O of their own.
...
- Any other `trutina-*` workspace package (`trutina-core`,
  ...
  `trutina-api`). This package is a dependency root; nothing here may depend
  on anything downstream of it.
- Any driver or client library for a specific backend (`pymongo`, `beanie`,
  `sqlalchemy`, `asyncpg`, an HTTP client, etc.) — this package describes
  configuration shape, it does not use the configuration to connect to
  anything.

## Layering rules

`trutina-config` has no internal layers of its own beyond four sibling files
(`mongo.py`, `postgres.py`, `api.py`, `base.py`) plus `__init__.py`. The only
ordering rule: `base.py` imports from `mongo.py`/`postgres.py`/`api.py` (to
nest them into `Settings`), never the reverse — a nested settings module must
never import the root `Settings`/`TestSettings` classes.
...
2. `pydantic-settings` reads the relevant dotenv file (if present) and
   ...
   variables.
3. Nested fields (`mongo`, `postgres`, `api`) are populated from
   double-underscore-delimited variables (`TRUTINA_MONGO__URI`,
   `TRUTINA_POSTGRES__URI`) or fall back to each nested model's own
   `Field(default=...)` values if unset.
4. The resulting `Settings`/`TestSettings` instance is handed to whatever
   ...
   `uvicorn.run(host=settings.api.host, ...)`), by the caller — this package
   never passes its own output anywhere itself.
...
`PostgresSettings` is a confirmed, already-realized example of the pattern
below (added alongside `MongoSettings`/`ApiSettings` during the Postgres
migration) — follow the same shape for anything added after it:
...
- This package's own test coverage is now confirmed present:
  `packages/config/src/trutina/config/tests/{test_mongo,test_api,
test_postgres,test_settings}.py` cover each nested settings group's
  defaults/overrides, confirm none is a `BaseSettings` subclass, and pin
  `Settings`/`TestSettings` namespace isolation and `get_settings()`'s cache
  behavior. This closes the prior version of this document's flag that test
  coverage "could not be confirmed from the source available for this
  review."
